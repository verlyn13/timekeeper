...
class TestAdaptiveAgentTemporal:
    ...
    def test_add_time_unit(self):
        ...
        # 1 epoch = 4 megacycles = 4 * 60 cycles = 240 cycles
        tp = self.adaptive.create_timepoint(epoch=1)
        base_units = self.adaptive.to_base_units(tp)
        expected = 1 * 4 * 60 * 1000 * 1000  # 1 epoch = 4 megacycles = 240 cycles = 14,400 steps = 14,400,000 microsteps
    ...
    def test_remove_time_unit(self):
        ...
        # Test with timepoints to verify conversion factors
        # Now 1 epoch = 1000*60 = 60,000 steps
        tp = self.adaptive.create_timepoint(epoch=1)
        base_units = self.adaptive.to_base_units(tp)
        expected = 1 * 1000 * 60 * 1000  # 1 epoch = 60,000 steps = 60,000,000 microsteps
        assert base_units == expected
