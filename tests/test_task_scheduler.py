...
class TestTaskScheduler:
    ...
    def test_simple_scheduling(self):
        ...
        # End time should be start + duration, after normalization
        # Duration = 100 steps = 100,000 microsteps
        # End time = (0e, 0c, 0s) + (0e, 0c, 100s) -> Normalized: (0e, 1c, 40s)
        end = scheduled[0]["end"]
        assert end["epoch"] == 0
        assert end["cycle"] == 1  # Corrected expectation
        assert end["step"] == 40  # Corrected expectation
        ...
        for i, task1 in enumerate(scheduled):
            for task2 in scheduled[i + 1 :]:
                if task1["agent"] == task2["agent"]:
                    ...
                    comparison2 = self.temporal.compare_timepoints(
                        task2["end"], task1["start"]
                    )
